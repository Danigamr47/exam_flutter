...
class _BillPaymentScreenState extends State<BillPaymentScreen> {
  ...
  final List<ServiceModel> _services = [
    ServiceModel(
      id: '1',
      name: 'SENELEC (Facture)',
      logoPath: '',
      color: Colors.orange,
    ),
    ServiceModel(
      id: '4',
      name: 'WOYOFAL (Achat crédit)',
      logoPath: '',
  ...
  IconData _getServiceIcon(String serviceName) {
    ...
    if (serviceName.contains("RAPIDO"))
      ...
      if (success) {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                "Paiement de ${_currencyFormat.format(amount)} pour ${service.name} réussi !",
              ),
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  keyboardType: service.name.contains("ISI")
                      ? TextInputType.text
                      : TextInputType.number,
                  decoration: InputDecoration(
                    labelText: service.name.contains("ISI")
                        ? "Matricule Étudiant"
                        : "Numéro de référence",
                    hintText: "Saisir les $requiredLength chiffres",
                    prefixIcon: Icon(
                      service.name.contains("ISI") ? Icons.school : Icons.tag,
                    ),
                    helperText: service.name.contains("ISI")
                        ? "Matricule de l'étudiant"
                        : "Référence client ou compteur",
                  ),
                  validator: (v) =>
                      v!.length != requiredLength ? "Référence invalide" : null,
                ),
                ...
                if (service.name.contains("ISI")) ...[
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    validator: (v) =>
                        v == null ? "Veuillez choisir le mois" : null,
                  ),
                ],
                      ...
                      onPressed: () {
                        if (_formKey.currentState!.validate()) {
                          ...
                          if (service.name.contains("ISI") &&
                              selectedMonth != null) {
                            finalReference =
                                "${refController.text} ($selectedMonth)";
                          }
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () =>
                              authProvider.toggleFavorite(service.id),
                        ),
